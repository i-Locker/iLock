...
export const getDeadline = (deadlineInMinutes) => Math.floor(new Date().getTime() + Number(deadlineInMinutes));
export const getOutPutDataFromEvent = async (tokenAddress, eventsArray, decimals) => {
    ...
    if (duplicateArray.length != 0) {
        ...
        const convertedInput = (parseInt(duplicateArray[0].data, 16) /
            10 ** seconddecimal).toFixed(7);
        return convertedInput;
    }
};
export const getInPutDataFromEvent = (tokenAddress, eventsArray, fromAmount, decimals) => {
    const duplicateArray = [];
    const decimal = decimals ? decimals : 16;
    const seconddecimal = decimals ? decimals : 18;
    eventsArray.map((event) => {
    ...
    if (duplicateArray.length !== 0) {
        const convertedInput = parseInt(duplicateArray[0].data, 16) / 10 ** seconddecimal;
