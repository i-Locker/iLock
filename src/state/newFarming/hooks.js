...
import { updateSearchResult, clearSearchResult, updateFilterResult, clearAllFarms, } from "./action";
import { useFarms } from "../farm/hooks";
import { useNewLPData } from "../LPFarm/hooks";
import { useActiveWeb3React } from "../../utils/hooks/useActiveWeb3React";
import { useWeb3React } from "@web3-react/core";
export const useSearch = () => {
    const farms = useSelector((state) => state.newFarming.filterResult);
    return farms;
};
export const useSearchResults = () => {
    const farms = useSelector((state) => state.newFarming);
    return farms;
};
export const useFilterFarms = ({ newestToOldest, min, max, setSavedChanges, saveChanges, }) => {
    const farmData = useFarms();
    const farms = useNewLPData();
    const { chainId } = useActiveWeb3React();
    const dispatch = useDispatch();
    const handleUpdateSearch = useCallback((searchResult) => {
    ...
    useMemo(() => {
        if (!newestToOldest && saveChanges) {
            ...
            const searchResult = dataArray.filter((item, idx) => item.APY > min && item.APY < max);
            ...
            const searchResult = dataArray.filter((item) => item.APY > min && item.APY < max);
            const editSearch = [...searchResult];
            handleUpdateSearch(editSearch);
            setSavedChanges(false);
        }
...
export const useFarmSearch = ({ keyword, previousKeyword, searchData, }) => {
    const FarmData = useFarms();
    ...
    useMemo(() => {
        if (keyword) {
            ...
            if (searchData) {
                ...
                const resultArray = dataArray.filter((item) => item.deposit.search(keyword) !== -1);
                const secondResultSearch = word.length >= 2 && resultArray.length === 0
                    ? dataArray.filter((item) => item.deposit.search(word[1] + "-" + word[0]) !== -1)
                    : undefined;
