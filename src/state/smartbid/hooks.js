...
export class BidStructure {
    constructor(time, exclusive, image, title, id, text, color, bgColor) {
        this.time = time;
...
export const useBidInfo = () => {
    const [bidItemsNFT, setBidItemsNFT] = useState([]);
    const { account, chainId, library } = useActiveWeb3React();
    const [loadBid, setLoadBid] = useState(false);
        ...
        const getSmartBidArray = async () => {
            try {
                ...
                for (let i = 0; i <= Number(item.toString()) - 1; i++) {
                    ...
                    const newData = new BidStructure(eventEnded, true, cardbid, 'NFT owners only', i, 'Participants get .. of their winnings.', '#F25F4C', '#FFF3F1');
                    setBidItemsNFT((prevState => [...prevState, newData]));
                }
                const bidContractTwo = await RigelSmartBidTwo(SMARTBID2[ChainId], lib);
                ...
                for (let i = 0; i <= Number(noNftLength.toString()) - 1; i++) {
                    ...
                    const bidDeadline = new Date(bidDate).getTime();
                    const eventEnded = bidDeadline < now;
                    const newData = new BidStructure(eventEnded, false, bidNFT, 'Everyone', i, 'NFT Owners get X2 of their winnings.', '#EDE8FD', '#EDE8FD');
                    setBidItemsNFT((prevState => [...prevState, newData]));
                }
                setLoadBid(false);
            }
