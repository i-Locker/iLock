...
import { MasterChefNEWLPContract, LiquidityPairInstance, smartFactory, productStakingContract, smartSwapLPTokenPoolThree, } from "../../utils/Contracts";
import { MASTERCHEFNEWLPADDRESSES, SMARTSWAPFACTORYADDRESSES, RGPADDRESSES, BUSD, USDT, USDC, PRODUCTSTAKINGADDRESSES, SMARTSWAPLP_TOKEN3ADDRESSES, SYMBOLS, } from "../../utils/addresses";
import { getERC20Token } from "../../utils/utilsFunctions";
import { ethers } from "ethers";
import { updateYieldFarmDetails, updateNewFilterResult, updateNewSearchResult, updateProductFarmDetails, } from "../newFarming/action";
import Web3 from "web3";
import { farmSection } from "../../pages/FarmingV2";
export const useNewLPData = () => {
    const farms = useSelector((state) => state.lpfarm);
    return farms;
};
const getBNBPrice = async (address, library) => {
    try {
        let BNBPrice;
        const pair = await smartSwapLPTokenPoolThree(address, library);
        ...
        if (pair.address === testNetPair) {
            BNBPrice = ethers.utils.formatUnits(reserves[0].mul(1000).div(reserves[1]), 3);
        }
        else {
            BNBPrice = ethers.utils.formatUnits(reserves[1].mul(1000).div(reserves[0]), 3);
        }
...
export const useUpdateNewFarm = ({ reload, setReload, content, contractID, section }) => {
    const data = useNewLPData();
    const searchSection = useSelector((state) => state.newFarming);
    const selectedField = useSelector((state) => state.farming.selectedField);
    ...
    const getLpTokenReserve = async (address) => {
        ...
        try {
            ...
            const [totalReserves, token0, token1, LPLockedPair, LPTotalSupply] = await Promise.all([
                ...
                LpTokenContract.balanceOf(MASTERCHEFNEWLPADDRESSES[chainId][contractID]),
    ...
    const calculateLiquidityAndApy = async (reward) => {
        try {
            const masterchef = await MasterChefNEWLPContract(MASTERCHEFNEWLPADDRESSES[chainId][contractID], library);
            ...
            const totalBNB = reserves
                ? SYMBOLS['BNB'][chainId] === reserves.tokenAddress0
                    ? reserves.reserves0.toString()
                    : reserves.reserves1.toString()
                : "1";
            const rgpPrice = await calculateRigelPrice();
            const BNBPrice = await getBNBPrice(SMARTSWAPLP_TOKEN3ADDRESSES[chainId], library);
            ...
            const [tokenEarned, userInfo, FarmTokenBalance, allowance] = await Promise.all([
                ...
                LPInstance.allowance(account, MASTERCHEFNEWLPADDRESSES[chainId][contractID]),
            ]);
...
export const useNewYieldFarmDetails = ({ content, section, loading, setLoading, contractID }) => {
    const data = useNewLPData();
    const searchSection = useSelector((state) => state.newFarming);
    const { account, chainId, library } = useWeb3React();
