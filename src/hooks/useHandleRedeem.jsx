import { CHAIN_ID_ALGORAND, CHAIN_ID_APTOS, CHAIN_ID_INJECTIVE, CHAIN_ID_KLAYTN, CHAIN_ID_NEAR, CHAIN_ID_SEI, CHAIN_ID_SOLANA, CHAIN_ID_SUI, CHAIN_ID_XPLA, isEVMChain, isTerraChain, postVaaSolanaWithRetry, redeemAndUnwrapOnSolana, redeemOnAlgorand, redeemOnEth, redeemOnEthNative, redeemOnInjective, redeemOnNear, redeemOnSolana, redeemOnSui, redeemOnTerra, redeemOnXpla, uint8ArrayToHex, } from "@certusone/wormhole-sdk";
...
import { ACALA_RELAY_URL, ALGORAND_BRIDGE_ID, ALGORAND_HOST, ALGORAND_TOKEN_BRIDGE_ID, MAX_VAA_UPLOAD_RETRIES_SOLANA, NEAR_TOKEN_BRIDGE_ACCOUNT, SEI_TRANSLATOR, SOLANA_HOST, SOL_BRIDGE_ADDRESS, SOL_TOKEN_BRIDGE_ADDRESS, getBridgeAddressForChain, getTokenBridgeAddressForChain, } from "../utils/consts";
...
async function algo(dispatch, enqueueSnackbar, senderAddr, signedVAA) {
    ...
    try {
        ...
        const txs = await redeemOnAlgorand(algodClient, ALGORAND_TOKEN_BRIDGE_ID, ALGORAND_BRIDGE_ID, signedVAA, senderAddr);
        const result = await signSendAndConfirmAlgorand(algodClient, txs);
        // TODO: fill these out correctly
        dispatch(setRedeemTx({
...
async function aptos(dispatch, enqueueSnackbar, signedVAA, signAndSubmitTransaction) {
    dispatch(setIsRedeeming(true));
    const tokenBridgeAddress = getTokenBridgeAddressForChain(CHAIN_ID_APTOS);
    try {
        const msg = await completeTransferAndRegister(getAptosClient(), tokenBridgeAddress, signedVAA);
        const result = await waitForSignAndSubmitTransaction(msg, signAndSubmitTransaction);
...
async function evm(dispatch, enqueueSnackbar, signer, signedVAA, isNative, chainId) {
    dispatch(setIsRedeeming(true));
    try {
...
async function near(dispatch, enqueueSnackbar, senderAddr, signedVAA, wallet) {
    ...
    try {
        ...
        const msgs = await redeemOnNear(makeNearProvider(), account.accountId, NEAR_TOKEN_BRIDGE_ACCOUNT, signedVAA);
...
async function terra(dispatch, enqueueSnackbar, wallet, signedVAA, feeDenom, chainId) {
    dispatch(setIsRedeeming(true));
    try {
        const msg = await redeemOnTerra(getTokenBridgeAddressForChain(chainId), wallet.terraAddress, signedVAA);
        const result = await postWithFees(wallet, [msg], "Wormhole - Complete Transfer", [feeDenom], chainId);
        dispatch(setRedeemTx({ id: result.result.txhash, block: result.result.height }));
        enqueueSnackbar(null, {
            content: <Alert severity="success">Transaction confirmed</Alert>,
        });
    }
...
async function xpla(dispatch, enqueueSnackbar, wallet, signedVAA) {
    dispatch(setIsRedeeming(true));
    try {
        const msg = await redeemOnXpla(getTokenBridgeAddressForChain(CHAIN_ID_XPLA), wallet.xplaAddress, signedVAA);
        const result = await postWithFeesXpla(wallet, [msg], "Wormhole - Complete Transfer");
        dispatch(setRedeemTx({ id: result.result.txhash, block: result.result.height }));
        enqueueSnackbar(null, {
            content: <Alert severity="success">Transaction confirmed</Alert>,
        });
...
async function injective(dispatch, enqueueSnackbar, wallet, walletAddress, signedVAA) {
    dispatch(setIsRedeeming(true));
    try {
        const msg = await redeemOnInjective(getTokenBridgeAddressForChain(CHAIN_ID_INJECTIVE), walletAddress, signedVAA);
        const tx = await broadcastInjectiveTx(wallet, walletAddress, msg, "Wormhole - Complete Transfer");
...
async function sei(dispatch, enqueueSnackbar, wallet, walletAddress, signedVAA) {
    dispatch(setIsRedeeming(true));
    try {
        const msg = {
        ...
        const tx = await wallet.execute(walletAddress, SEI_TRANSLATOR, msg, fee, "Wormhole - Complete Transfer");
...
async function sui(dispatch, enqueueSnackbar, wallet, signedVAA) {
    dispatch(setIsRedeeming(true));
    try {
        const provider = getSuiProvider();
        const tx = await redeemOnSui(provider, getBridgeAddressForChain(CHAIN_ID_SUI), getTokenBridgeAddressForChain(CHAIN_ID_SUI), signedVAA);
...
export function useHandleRedeem() {
    ...
    const handleRedeemClick = useCallback(() => {
        if (isEVMChain(targetChain) && !!signer && signedVAA) {
            ...
            !!solPK &&
            signedVAA) {
            solana(dispatch, enqueueSnackbar, solanaWallet, solPK.toString(), signedVAA, false);
        }
        else if (isTerraChain(targetChain) && !!terraWallet && signedVAA) {
            terra(dispatch, enqueueSnackbar, terraWallet, signedVAA, terraFeeDenom, targetChain);
        }
            ...
            seiAddress &&
            signedVAA) {
            sei(dispatch, enqueueSnackbar, seiSigningCosmWasmClient, seiAddress, signedVAA);
        }
    ...
    const handleRedeemNativeClick = useCallback(() => {
        ...
        if (isEVMChain(targetChain) && !!signer && signedVAA) {
            ...
            !!solPK &&
            signedVAA) {
            solana(dispatch, enqueueSnackbar, solanaWallet, solPK.toString(), signedVAA, true);
        }
        else if (isTerraChain(targetChain) && !!terraWallet && signedVAA) {
            terra(dispatch, enqueueSnackbar, terraWallet, signedVAA, terraFeeDenom, targetChain); //TODO isNative = true
        }
        else if (targetChain === CHAIN_ID_ALGORAND &&
            algoAccounts[0] &&
            !!signedVAA) {
