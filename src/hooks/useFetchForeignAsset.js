import { CHAIN_ID_ALGORAND, CHAIN_ID_APTOS, CHAIN_ID_INJECTIVE, CHAIN_ID_NEAR, CHAIN_ID_SEI, CHAIN_ID_SOLANA, CHAIN_ID_SUI, CHAIN_ID_TERRA2, CHAIN_ID_XPLA, getEmitterAddressNear, getForeignAssetAlgorand, getForeignAssetAptos, getForeignAssetEth, getForeignAssetInjective, getForeignAssetNear, getForeignAssetSolana, getForeignAssetSui, getForeignAssetTerra, getForeignAssetXpla, hexToUint8Array, isEVMChain, isTerraChain, nativeToHexString, } from "@certusone/wormhole-sdk";
...
import { ALGORAND_HOST, ALGORAND_TOKEN_BRIDGE_ID, NATIVE_NEAR_PLACEHOLDER, NATIVE_NEAR_WH_ADDRESS, NEAR_TOKEN_BRIDGE_ACCOUNT, SOLANA_HOST, SOL_TOKEN_BRIDGE_ADDRESS, XPLA_LCD_CLIENT_CONFIG, getEvmChainId, getTerraConfig, getTokenBridgeAddressForChain, } from "../utils/consts";
...
function useFetchForeignAsset(originChain, originAsset, foreignChain) {
    ...
    const argumentError = useMemo(() => !originChain ||
        !originAsset ||
        !foreignChain ||
        !originAssetHex ||
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        try {
            const getterFunc = isEVMChain(foreignChain)
                ? () => getForeignAssetEth(getTokenBridgeAddressForChain(foreignChain), provider, //why does this typecheck work elsewhere?
                originChain, hexToUint8Array(originAssetHex))
                : isTerraChain(foreignChain)
                    ? () => {
                        const lcd = new LCDClient(getTerraConfig(foreignChain));
                        return getForeignAssetTerra(getTokenBridgeAddressForChain(foreignChain), lcd, originChain, hexToUint8Array(originAssetHex));
                    }
                    : foreignChain === CHAIN_ID_XPLA
                        ? () => {
                            const lcd = new XplaLCDClient(XPLA_LCD_CLIENT_CONFIG);
                            return getForeignAssetXpla(getTokenBridgeAddressForChain(foreignChain), lcd, originChain, hexToUint8Array(originAssetHex));
                        }
                        : foreignChain === CHAIN_ID_APTOS
                            ? () => {
                                return getForeignAssetAptos(getAptosClient(), getTokenBridgeAddressForChain(foreignChain), originChain, originAssetHex);
                            }
                            : foreignChain === CHAIN_ID_SOLANA
                                ? () => {
                                    const connection = new Connection(SOLANA_HOST, "confirmed");
                                    return getForeignAssetSolana(connection, SOL_TOKEN_BRIDGE_ADDRESS, originChain, hexToUint8Array(originAssetHex));
                                }
                                : foreignChain === CHAIN_ID_ALGORAND
                                    ? () => {
                                        ...
                                        return getForeignAssetAlgorand(algodClient, ALGORAND_TOKEN_BRIDGE_ID, originChain, originAssetHex);
                                    }
                                    : foreignChain === CHAIN_ID_INJECTIVE
                                        ? () => {
                                            const client = getInjectiveWasmClient();
                                            return getForeignAssetInjective(getTokenBridgeAddressForChain(foreignChain), client, originChain, hexToUint8Array(originAssetHex));
                                        }
                                        : foreignChain === CHAIN_ID_SEI
                                            ? async () => {
                                                const client = await getSeiWasmClient();
                                                return getForeignAssetSei(getTokenBridgeAddressForChain(foreignChain), client, originChain, hexToUint8Array(originAssetHex));
                                            }
                                            : foreignChain === CHAIN_ID_NEAR
                                                ? async () => {
                                                    try {
                                                        return await getForeignAssetNear(makeNearProvider(), NEAR_TOKEN_BRIDGE_ACCOUNT, originChain, originAssetHex);
                                                    }
                                                    catch {
                                                ...
                                                : foreignChain === CHAIN_ID_SUI
                                                    ? () => {
                                                        return getForeignAssetSui(getSuiProvider(), getTokenBridgeAddressForChain(CHAIN_ID_SUI), originChain, hexToUint8Array(originAssetHex));
                                                    }
    ...
    const output = useMemo(() => ({
        ...
        data: (assetAddress !== null && assetAddress !== undefined) ||
