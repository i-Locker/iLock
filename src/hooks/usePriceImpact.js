...
export const useCalculatePriceImpact = (routeAddress, amountIn, fromAmount, currencyA, currencyB) => {
    ...
    useMemo(async () => {
        if (routeAddress && amountIn && fromAmount && account) {
            const rout = await SmartSwapRouter(SMARTSWAPROUTER[chainId], lib);
            ...
            if (routeAddress.length === 2) {
                try {
                    ...
                    const marketPrice = ethers.utils.formatUnits(price[1].toString(), currencyB.decimals);
                ...
                try {
                    ...
                    const marketPrice = ethers.utils.formatUnits(price2[1].toString(), currencyB.decimals);
                    const swapPrice = amountIn / fromAmount;
                    const priceDifference = swapPrice - parseFloat(marketPrice);
                    const priceImpact = (priceDifference / parseFloat(marketPrice)) * 100;
                    setPriceImpact(priceImpact.toFixed(2));
                ...
                try {
                    ...
                    const marketPrice = ethers.utils.formatUnits(price3[1].toString(), currencyB.decimals);
                    const swapPrice = amountIn / fromAmount;
                    const priceDifference = swapPrice - parseFloat(marketPrice);
                    const priceImpact = (priceDifference / parseFloat(marketPrice)) * 100;
                    setPriceImpact(priceImpact.toFixed(2));
