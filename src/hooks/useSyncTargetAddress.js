import { cosmos, CHAIN_ID_ALGORAND, CHAIN_ID_APTOS, CHAIN_ID_NEAR, CHAIN_ID_SOLANA, CHAIN_ID_XPLA, isEVMChain, isTerraChain, uint8ArrayToHex, CHAIN_ID_INJECTIVE, CHAIN_ID_SEI, CHAIN_ID_SUI, } from "@certusone/wormhole-sdk";
...
import { selectNFTTargetAsset, selectNFTTargetChain, selectTransferTargetAsset, selectTransferTargetChain, selectTransferTargetParsedTokenAccount, } from "../store/selectors";
...
function useSyncTargetAddress(shouldFire, nft) {
    ...
    const targetAsset = useSelector(nft ? selectNFTTargetAsset : selectTransferTargetAsset);
    ...
    useEffect(() => {
        ...
        if (shouldFire) {
            ...
            if (isEVMChain(targetChain) && signerAddress) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(arrayify(signerAddress), 32))));
            }
            // TODO: have the user explicitly select an account on solana
            else if (!nft && // only support existing, non-derived token accounts for token transfers (nft flow doesn't check balance)
                targetChain === CHAIN_ID_SOLANA &&
                targetTokenAccountPublicKey) {
                // use the target's TokenAccount if it exists
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(new PublicKey(targetTokenAccountPublicKey).toBytes(), 32))));
            }
                ...
                (async () => {
                    try {
                        const associatedTokenAccount = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, new PublicKey(targetAsset), // this might error
                        solPK);
                        if (!cancelled) {
                            dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(associatedTokenAccount.toBytes(), 32))));
                        }
                    }
                    catch (e) {
                        if (!cancelled) {
                ...
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(cosmos.canonicalAddress(terraWallet.walletAddress), 32))));
            }
            else if (targetChain === CHAIN_ID_XPLA &&
                xplaWallet &&
                xplaWallet.walletAddress) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(cosmos.canonicalAddress(xplaWallet.walletAddress), 32))));
            }
            else if (targetChain === CHAIN_ID_APTOS && aptosAddress) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(aptosAddress, 32))));
            }
            else if (targetChain === CHAIN_ID_ALGORAND && algoAccounts[0]) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(decodeAddress(algoAccounts[0].address).publicKey)));
            }
            else if (targetChain === CHAIN_ID_INJECTIVE && injAddress) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(cosmos.canonicalAddress(injAddress), 32))));
            }
            else if (targetChain === CHAIN_ID_SEI && seiAddress) {
                dispatch(setTargetAddressHex(uint8ArrayToHex(zeroPad(cosmos.canonicalAddress(seiAddress), 32))));
            }
                ...
                (async () => {
                    try {
                        ...
                        let account_hash = await account.viewFunction(NEAR_TOKEN_BRIDGE_ACCOUNT, "hash_account", {
                        ...
                        if (!cancelled) {
                            ...
                            if (!account_hash[0]) {
                                ...
                                let myAddress2 = getTransactionLastResult(await signAndSendTransactions(account, nearWallet, [
                                    {
