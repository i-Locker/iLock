import { CHAIN_ID_SOLANA, CHAIN_ID_TERRA, isEVMChain, isTerraChain, } from "@certusone/wormhole-sdk";
...
import { getDefaultNativeCurrencySymbol, SOLANA_HOST, getTerraConfig, } from "../utils/consts";
...
const TERRA_THRESHOLD_ULUNA = BigInt(100000);
const TERRA_THRESHOLD_UUSD = BigInt(10000000);
const isSufficientBalance = (chainId, balance, terraFeeDenom) => {
...
const isSufficientBalanceTerra = (balances) => {
...
const getBalanceSolana = async (walletAddress) => {
...
const getBalanceEvm = async (walletAddress, provider) => {
    return provider.getBalance(walletAddress).then((result) => result.toBigInt());
};
const getBalancesTerra = async (walletAddress, chainId) => {
...
const toBalanceString = (balance, chainId) => {
...
export default function useTransactionFees(chainId) {
    const { walletAddress, isReady } = useIsWalletReady(chainId);
    const { provider } = useEthereumProvider();
    const [balance, setBalance] = useState(undefined);
    const [terraBalances, setTerraBalances] = useState([]);
    ...
    useEffect(() => {
        if (chainId === CHAIN_ID_SOLANA && isReady && walletAddress) {
            loadStart();
            getBalanceSolana(walletAddress).then((result) => {
                const adjustedresult = result === undefined || result === null ? BigInt(0) : result;
                setIsLoading(false);
            ...
            if (provider) {
                loadStart();
                getBalanceEvm(walletAddress, provider).then((result) => {
                    const adjustedresult = result === undefined || result === null ? BigInt(0) : result;
                    setIsLoading(false);
                    setBalance(adjustedresult);
                }, (error) => {
            ...
            loadStart();
            getBalancesTerra(walletAddress, chainId).then((results) => {
                const adjustedResults = results.map(({ denom, balance }) => {
                    return {
                        denom,
                        balance: balance === undefined || balance === null ? BigInt(0) : balance,
                    };
                });
                setIsLoading(false);
...
export function useEthereumGasPrice(contract, chainId) {
    const { provider } = useEthereumProvider();
    ...
    useEffect(() => {
        if (provider && isReady && !estimateResults) {
            getGasEstimates(provider, contract).then((results) => {
                setEstimateResults(results);
                if (results?.currentGasPrice) {
                    const gasPrice = (results?.currentGasPrice &&
...
function EthGasEstimateSummary({ methodType, chainId, priceQuote, }) {
...
export async function getGasEstimates(provider, contract) {
    const lowEstimateGasAmount = evmEstimatesByContract[contract].lowGasEstimate;
    const highEstimateGasAmount = evmEstimatesByContract[contract].highGasEstimate;
    let lowEstimate;
    let highEstimate;
    let currentGasPrice;
    if (provider) {
        ...
        if (priceInWei) {
            lowEstimate = parseFloat(formatUnits(lowEstimateGasAmount * priceInWei.toBigInt(), "ether")).toFixed(4);
            highEstimate = parseFloat(formatUnits(highEstimateGasAmount * priceInWei.toBigInt(), "ether")).toFixed(4);
            const gasPriceNum = parseFloat(formatUnits(priceInWei, "gwei"));
            currentGasPrice = gasPriceNum.toFixed(0);
        }
    }
    const output = currentGasPrice && highEstimate && lowEstimate
        ? {
            currentGasPrice,
            lowEstimate,
            highEstimate,
...
function TerraGasEstimateSummary({ methodType }) {
    if (methodType === "transfer") {
        const lowEstimate = formatUnits(terraEstimatesByContract.transfer.lowGasEstimate, NATIVE_TERRA_DECIMALS);
        const highEstimate = formatUnits(terraEstimatesByContract.transfer.highGasEstimate, NATIVE_TERRA_DECIMALS);
        return (<Typography component="div" style={{
                display: "flex",
                alignItems: "center",
                marginTop: 8,
...
export function GasEstimateSummary({ methodType, chainId, priceQuote, //this is a hack, should refactor to unify the fee selector and this file
 }) {
    if (isEVMChain(chainId)) {
        return (<EthGasEstimateSummary chainId={chainId} methodType={methodType} priceQuote={priceQuote}/>);
    }
