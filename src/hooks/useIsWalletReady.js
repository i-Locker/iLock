...
function useIsWalletReady(chainId, enableNetworkAutoswitch = true) {
    const autoSwitch = enableNetworkAutoswitch;
    const solanaWallet = useSolanaWallet();
    const solPK = solanaWallet?.publicKey;
    const terraWallet = useConnectedWallet();
    const hasTerraWallet = !!terraWallet;
    const { provider, signerAddress, chainId: evmChainId, connectType, disconnect, } = useEthereumProvider();
        ...
        if (provider && correctEvmNetwork) {
            ...
            try {
                ...
                if (switchError.code === 4902) {
                    const addChainParameter = METAMASK_CHAIN_PARAMETERS[correctEvmNetwork];
    ...
    return useMemo(() => {
        if (isTerraChain(chainId) && hasTerraWallet && terraWallet?.walletAddress) {
            ...
            return createWalletStatus(true, undefined, forceNetworkSwitch, terraWallet.walletAddress);
        }
        if (chainId === CHAIN_ID_SOLANA && solPK) {
            return createWalletStatus(true, undefined, forceNetworkSwitch, solPK.toString());
        }
        ...
        if (chainId === CHAIN_ID_XPLA &&
            ...
            return createWalletStatus(true, undefined, forceNetworkSwitch, xplaWallet.walletAddress);
        }
        if (chainId === CHAIN_ID_APTOS && hasAptosWallet && aptosAddress) {
            if (hasCorrectAptosNetwork) {
                return createWalletStatus(true, undefined, forceNetworkSwitch, aptosAddress);
            }
            else {
                return createWalletStatus(false, `Wallet is not connected to ${APTOS_NETWORK}.`, forceNetworkSwitch, undefined);
            }
        }
        if (chainId === CHAIN_ID_INJECTIVE && hasInjWallet && injAddress) {
            return createWalletStatus(true, undefined, forceNetworkSwitch, injAddress);
        }
        if (chainId === CHAIN_ID_SEI && hasSeiWallet && seiAddress) {
            return createWalletStatus(true, undefined, forceNetworkSwitch, seiAddress);
        }
        ...
        if (chainId === CHAIN_ID_SUI && suiAddress) {
            return createWalletStatus(true, undefined, forceNetworkSwitch, suiAddress);
        }
        if (isEVMChain(chainId) && hasEthInfo && signerAddress) {
            if (hasCorrectEvmNetwork) {
                ...
                return createWalletStatus(false, `Wallet is not connected to ${CLUSTER}. Expected Chain ID: ${correctEvmNetwork}`, forceNetworkSwitch, undefined);
            }
        }
        return createWalletStatus(false, "Wallet not connected", forceNetworkSwitch, undefined);
    }, [
        chainId,
        autoSwitch,
        forceNetworkSwitch,
