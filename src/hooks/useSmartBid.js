...
export const useSmartBid = (id, exclusive) => {
        ...
        const fetchBidData = async () => {
            if (exclusive) {
                if (ChainId !== undefined) {
                    try {
                        const bidContract = await RigelSmartBid(SMARTBID1[ChainId], lib);
                ...
                if (ChainId !== undefined) {
                    try {
                        const bidContract = await RigelSmartBidTwo(SMARTBID2[ChainId], lib);
                        const bidData = await bidContract.request_data_in_Bidding(id);
                        setBidTime(bidData.timeOut.toString());
                        setAddresses(bidData.numberOfRandomAddress.toString());
                        const maxOutput = Number(bidData.highestbid.toString()) !== 0 ?
...
export const useBidAllowance = (checkTokenApproval, amount, isOpen, id, exclusive) => {
        ...
        const getAllowance = async () => {
            if (account) {
                if (exclusive) {
                    try {
                        setLoadInfo(true);
                        const bidContract = await RigelSmartBid(SMARTBID1[chainId], library);
                        ...
                        const [allowanceA] = await Promise.all([
                            tokenA.allowance(account, SMARTBID1[chainId])
                        ]);
                    ...
                    try {
                        setLoadInfo(true);
                        const bidContract = await RigelSmartBidTwo(SMARTBID2[chainId], library);
                        const bidToken = await bidContract.requestToken(id);
                        const [tokenA] = await Promise.all([
                            getERC20Token(bidToken._token, library)
                        ]);
                        const [allowanceA] = await Promise.all([
                            tokenA.allowance(account, SMARTBID2[chainId])
                        ]);
                        const isTokenAApproved = allowanceA.toString() > parseFloat(amount.toString());
                        setHasTokenABeenApproved(isTokenAApproved);
                        setLoadInfo(false);
...
export const useBidWinners = (id, exclusive) => {
    const { chainId, library, account } = useActiveWeb3React();
    const [loadWinners, setLoadWinners] = useState(false);
    const [rewardArray, setRewardArray, setRewardArrayRef] = useState([]);
    const [totalBid, setTotalBid] = useState('');
    const [winnerDetails, setWinnerDetails] = useState(SmartBidWinners);
