...
import { IPFS, SMARTSWAPNFTSALES } from "../utils/addresses";
...
export const useNft = (id) => {
    ...
    const [firstToken, setFirstToken] = useState({ symbol: '', balance: '', address: '' });
    const [secondToken, setSecondToken] = useState({ symbol: '', balance: '', address: '' });
    const [prices, setPrices] = useState({ firstTokenPrice: '', secondTokenPrice: '' });
    const [unsoldItems, setUnsoldItems] = useState(0);
    const [nftId, setNftId, nftIdRef] = useState([]);
    const [nftClass, setNftClass, nftClassRef] = useState();
    ...
    if (SMARTSWAPNFTSALES[chainId] !== "0x") {
        validSmartAddress = SMARTSWAPNFTSALES[chainId];
    }
    useEffect(() => {
        setLoadData(true);
        ...
        const fetchNftData = async () => {
            if (chainId !== undefined) {
                ...
                const [tokenOneSymbol, tokenOneBalance, tokenOneDecimals] = await Promise.all([tokenOne.symbol(), tokenOne.balanceOf(account), tokenOne.decimals()]);
                ...
                const [tokenTwoSymbol, tokenTwoBalance, tokenTwoDecimals] = await Promise.all([tokenTwo.symbol(), tokenTwo.balanceOf(account), tokenTwo.decimals()]);
...
export const useNFTAllowance = (checkTokenApproval, tokenPrice, currency, id, tokenA, tokenB) => {
                ...
                try {
                    ...
                    if (tokenA && tokenB) {
                        const [tokenAData, tokenBData] = await Promise.all([
                            getERC20Token(tokenA, library),
                            getERC20Token(tokenB, library),
                        ]);
                        const [allowanceA, allowanceB, decimalsA, decimalsB] = await Promise.all([
                            tokenAData.allowance(account, SMARTSWAPNFTSALES[chainId]),
                            tokenBData.allowance(account, SMARTSWAPNFTSALES[chainId]),
                            tokenAData.decimals(),
                            tokenBData.decimals()
                        ]);
...
export const useNftName = (id, nftID) => {
    const { chainId, library } = useActiveWeb3React();
    const [name, setName] = useState('');
    const [nftImage, setNftImage] = useState('');
    const [loading, setLoading] = useState(false);
    const [nftId, setNftId, nftIdRef] = useState([]);
    const ChainId = useSelector((state) => state.newfarm.chainId);
