import { CHAIN_ID_ALGORAND, CHAIN_ID_APTOS, CHAIN_ID_INJECTIVE, CHAIN_ID_NEAR, CHAIN_ID_SEI, CHAIN_ID_SOLANA, CHAIN_ID_SUI, CHAIN_ID_XPLA, ensureHexPrefix, ethers_contracts, isEVMChain, isNativeDenomInjective, isNativeDenomXpla, isTerraChain, parseSmartContractStateResponse, terra, } from "@certusone/wormhole-sdk";
...
import { ALGORAND_HOST, NATIVE_NEAR_DECIMALS, NATIVE_NEAR_PLACEHOLDER, SOLANA_HOST, XPLA_LCD_CLIENT_CONFIG, getEvmChainId, getTerraConfig, } from "../utils/consts";
import { NATIVE_INJECTIVE_DECIMALS, getInjectiveBankClient, getInjectiveWasmClient, } from "../utils/injective";
...
function useGetTargetParsedTokenAccounts() {
    ...
    const tokenName = (targetAsset && metadata.data?.get(targetAsset)?.tokenName) || undefined;
    const symbol = (targetAsset && metadata.data?.get(targetAsset)?.symbol) || undefined;
    const logo = (targetAsset && metadata.data?.get(targetAsset)?.logo) || undefined;
    const decimals = (targetAsset && metadata.data?.get(targetAsset)?.decimals) || undefined;
    ...
    useEffect(() => {
            ...
            if (terra.isNativeDenom(targetAsset)) {
                lcd.bank
                    ...
                    if (balance && !cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", balance, NATIVE_TERRA_DECIMALS, Number(formatUnits(balance, NATIVE_TERRA_DECIMALS)), formatUnits(balance, NATIVE_TERRA_DECIMALS), symbol, tokenName, logo)));
                    }
                })
                    .catch(() => {
                    if (!cancelled) {
                ...
                lcd.wasm
                    ...
                    if (balance && info && !cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", balance.balance.toString(), info.decimals, Number(formatUnits(balance.balance, info.decimals)), formatUnits(balance.balance, info.decimals), symbol, tokenName, logo)));
                    }
                }))
                    .catch(() => {
                    if (!cancelled) {
        ...
        if (targetChain === CHAIN_ID_XPLA && xplaWallet) {
            ...
            if (isNativeDenomXpla(targetAsset)) {
                lcd.bank
                    ...
                    const balance = coins.get(targetAsset)?.amount?.toString();
                    if (balance && !cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", balance, NATIVE_XPLA_DECIMALS, Number(formatUnits(balance, NATIVE_XPLA_DECIMALS)), formatUnits(balance, NATIVE_XPLA_DECIMALS), symbol, tokenName, logo)));
                    }
                })
                    .catch(() => {
                    if (!cancelled) {
            ...
            (async () => {
                try {
                    ...
                    try {
                        ...
                        const coinStore = `0x1::coin::CoinStore<${ensureHexPrefix(targetAsset)}>`;
                        value = (await client.getAccountResource(aptosAddress, coinStore))
                            .data.coin.value;
                    }
                    catch (e) { }
                    if (!cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", value.toString(), decimals, Number(formatUnits(value, decimals)), formatUnits(value, decimals), symbol, tokenName, logo)));
                    }
                }
                catch (e) {
                    if (!cancelled) {
        ...
        if (targetChain === CHAIN_ID_INJECTIVE && injAddress) {
            if (isNativeDenomInjective(targetAsset)) {
                ...
                client
                    ...
                    .then(({ amount }) => {
                    if (!cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", amount, NATIVE_INJECTIVE_DECIMALS, Number(formatUnits(amount, NATIVE_INJECTIVE_DECIMALS)), formatUnits(amount, NATIVE_INJECTIVE_DECIMALS), symbol, tokenName, logo)));
                    }
                })
                    .catch(() => {
                    if (!cancelled) {
                ...
                client
                    .fetchSmartContractState(targetAsset, Buffer.from(JSON.stringify({
                    token_info: {},
                })).toString("base64"))
                    .then((infoData) => client
                    .fetchSmartContractState(targetAsset, Buffer.from(JSON.stringify({
                    balance: {
                        address: injAddress,
                    },
        ...
        if (targetChain === CHAIN_ID_SUI && suiAddress) {
            ...
            (async () => {
                try {
                    ...
                    if (!cancelled) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount("", "", totalBalance, decimals, Number(formatUnits(totalBalance, decimals)), formatUnits(totalBalance, decimals), symbol, tokenName, logo)));
                    }
                }
                catch (e) {
        ...
        if (targetChain === CHAIN_ID_SOLANA && solPK) {
            ...
            connection
                ...
                if (!cancelled) {
                    if (value.length) {
                        dispatch(setTargetParsedTokenAccount(createParsedTokenAccount(value[0].pubkey.toString(), value[0].account.data.parsed?.info?.mint, value[0].account.data.parsed?.info?.tokenAmount?.amount, value[0].account.data.parsed?.info?.tokenAmount?.decimals, value[0].account.data.parsed?.info?.tokenAmount?.uiAmount, value[0].account.data.parsed?.info?.tokenAmount
                            ?.uiAmountString, symbol, tokenName, logo)));
                    }
                    else {
                        // TODO: error state
                    }
        ...
        if (isEVMChain(targetChain) &&
            ...
            token
                ...
                token.balanceOf(signerAddress).then((n) => {
                    if (!cancelled) {
                        dispatch(setTargetParsedTokenAccount(
                        // TODO: verify accuracy
                        createParsedTokenAccount(signerAddress, token.address, n.toString(), decimals, Number(formatUnits(n, decimals)), formatUnits(n, decimals), symbol, tokenName, logo)));
                    }
                });
            })
                .catch(() => {
        ...
        if (targetChain === CHAIN_ID_ALGORAND &&
            algoAccounts[0] &&
            decimals !== undefined) {
            ...
            try {
                ...
                algodClient
                    ...
                    dispatch(setTargetParsedTokenAccount(createParsedTokenAccount(algoAccounts[0].address, targetAsset, balance.toString(), decimals, Number(formatUnits(balance, decimals)), formatUnits(balance, decimals), symbol, tokenName, logo)));
                })
                    .catch(() => {
                    if (!cancelled) {
                        // TODO: error state
                ...
                makeNearAccount(nearAccountId)
                    ...
                    if (targetAsset === NATIVE_NEAR_PLACEHOLDER) {
                        account
                            ...
                            .then((balance) => {
                            if (!cancelled) {
                                dispatch(setTargetParsedTokenAccount(createParsedTokenAccount(nearAccountId, //publicKey
                        ...
                        fetchSingleMetadata(targetAsset, account)
                            ...
                            account
                                ...
                                .then((balance) => {
                                if (!cancelled) {
                                    dispatch(setTargetParsedTokenAccount(createParsedTokenAccount(nearAccountId, targetAsset, balance.toString(), decimals, Number(formatUnits(balance, decimals)), formatUnits(balance, decimals), symbol, tokenName, logo)));
                                }
                            })
                                .catch(() => {
                                if (!cancelled) {
