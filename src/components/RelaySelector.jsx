...
export default function RelaySelector({ selectedValue, onChange, }) {
    ...
    const selector = (<TextField onChange={onChangeWrapper} value={selectedValue ? selectedValue.url : ""} label="Select a relayer" select fullWidth>
      {availableRelayers.data?.relayers?.map((item) => (<MenuItem key={item.url} value={item.url}>
          {item.name}
        </MenuItem>))}
    </TextField>);
    const error = (<Typography variant="body2" color="textSecondary">
