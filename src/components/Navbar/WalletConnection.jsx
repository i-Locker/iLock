...
import { Flex, Text, Button, useColorModeValue, useMediaQuery } from '@chakra-ui/react';
...
export default function WalletConnection() {
    ...
    const shadow = useColorModeValue('0px 1px 7px -2px rgba(24, 39, 75, 0.06), 0px 2px 2px rgba(24, 39, 75, 0.06)', '0px 2px 4px -2px rgba(178, 193, 230, 0.12), 0px 4px 4px -2px rgba(178, 193, 230, 0.08)');
    ...
    if (account) {
        return (<>
        <Button display={isMobileDevice ? 'none' : undefined} variant="rgpButton" bg={bgColor} onClick={() => {
            GRGPBreakdown();
            setShowRGP(true);
        }} fontSize="14px">
        ...
        <Flex ml={2} w={isMobileDevice ? '160px' : 'max-content'} borderRadius="md" border={'1px solid'} borderColor={bgColor2} h="10" justify="space-between">
          <Flex display={isMobileDevice ? 'none' : undefined} align="center" justify="center" bg={bgColor2} px={2}>
            <Text fontWeight={'bold'} fontSize="14px">
              {Balance} {Symbol}
            </Text>
          </Flex>
          <Button onClick={() => setDisplayWallet((state) => !state)} variant={'ghost'} fontSize="14px" rightIcon={<StatusIcon connector={connector}/>}>
        ...
        return (<>{error.name === "UnsupportedChainIdError" ? <> <Button bg="red.300" rightIcon={<IoWalletOutline />} variant="brand" onClick={() => setDisplayModal(state => !state)}>
        Unsupported Network
      </Button>
        ...
        return (<>
        <Button data-tut="reactour__WalletConnect" onClick={() => {
            setDisplayNetwork(state => !state);
            localStorage.removeItem('walletconnect');
        }} rightIcon={<IoWalletOutline />} variant="brand">
          Connect Wallet
        </Button>
