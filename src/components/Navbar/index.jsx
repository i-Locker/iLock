...
import { Flex, Spacer, Box, Img, useColorModeValue, useMediaQuery, Text, Tooltip, } from "@chakra-ui/react";
...
export const Nav = ({ to, label, active, img, }) => {
    const { chainId } = useActiveWeb3React();
    const mobileNavColor = useColorModeValue("#FFFFFF", "#15202B");
    return (<Tooltip label={chainId === SupportedChainId.AVALANCHE || chainId === SupportedChainId.AVALANCHE_FUJI && "coming soon"} bg="gray.300" color="black">
    <NavLink to={to} activeStyle={{
        color: "#319EF6",
    }} color={active ? "#319EF6" : mobileNavColor} style={active ? { color: "#319EF6" } : { color: "" }}>
      <Flex>
        {img}{" "}
...
const Index = () => {
    ...
    const { chainId } = useActiveWeb3React();
    return (<>
      {location === "/" ? null : (<Flex px={6} py={2} boxShadow="sm">
          {isMobileDevice ? (<>
              ...
              <Flex h="70px" zIndex="200" position="fixed" left={0} bottom={0} justify="space-between" alignItems="center" borderTop="1px" borderColor={mobileNavBorderColor} w="100%" bgColor={mobileNavColor} mr={4}>
                <Flex ml={4}>
                  <WalletConnection />
                </Flex>
                <Flex mr={4}>
              ...
              <Flex h="10">
                ...
                <Flex mr="4px" w="400px" h="10" align="center" justify="space-around" fontSize="14px" className="HeaderRide">
                  <SwapDropdown />
                  <BridgeDropdown />
                  <EarnDropdown />
