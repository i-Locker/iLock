...
import { Flex, useDisclosure, Drawer, DrawerOverlay, DrawerHeader, DrawerBody, DrawerContent, useColorModeValue, Text, Link, Collapse, useMediaQuery, Img, } from "@chakra-ui/react";
...
const MobileNavDrawer = () => {
    ...
    return (<>
      <Flex border="1px" borderColor={HamburgerIconBorderColor} alignItems="center" borderRadius="6px" w="40px" h="40px" justifyContent="center" p="18px">
        <HamburgerIcon color={HamburgerIconColor} onClick={onOpen} cursor={"pointer"} w="24px" h="24px"/>
      </Flex>
      <Drawer placement="left" onClose={onClose} isOpen={isOpen}>
        ...
        <DrawerContent>
          <DrawerHeader mt={5}>
            <Flex onClick={onClose} alignItems="center" cursor="pointer" justifyContent="center" borderRadius="6px" w="24px" h="24px" border="1px" borderColor={closeButtonBorder}>
              <CloseIcon />
            </Flex>
          </DrawerHeader>
          <DrawerBody>
        ...
        color: "#319EF6",
                  ...
                  <Flex mb={2} alignItems="center" w="320px" h="40px" justifyContent="space-between" onClick={toggleIsOn} bgColor={isOn ? SwapBgColor : "transparent"}>
                    <Flex ml={6}>
                      <Nav label="Swap" to={chainId === 43114 ? "/freeswap" : "/swap"} img={<Img src={SwapIcon}/>}/>
                    </Flex>
                    {isOn ? (<ChevronUpIcon mr={2}/>) : (<ChevronDownIcon mr={2}/>)}
                  </Flex>
                </NavLink>
                <Collapse animateOpacity in={isOn}>
                  <Flex display={isOn ? "" : "none"} flexDirection="column" ml={10} mb={3}>
        ...
        color: "#319EF6",
                  ...
                  <Flex mb={2} alignItems="center" w="320px" h="40px" justifyContent="space-between" onClick={toggleIsOnFarm} bgColor={isOnFarm ? SwapBgColor : "transparent"}>
                    <Flex ml={6}>
                      <Nav label="Farm" to="/swap" img={<Img src={FarmingIcon}/>}/>
                    </Flex>
                    {isOnFarm ? (<ChevronUpIcon mr={2}/>) : (<ChevronDownIcon mr={2}/>)}
                  </Flex>
                </NavLink>
                <Collapse animateOpacity in={isOnFarm}>
                  <Flex display={isOnFarm ? "" : "none"} flexDirection="column" ml={10} mb={3}>
        ...
        color: "#319EF6",
                  ...
                  <Flex mb={2} alignItems="center" w="320px" h="40px" justifyContent="space-between" onClick={toggleIsOnBridge} bgColor={isOnBridge ? SwapBgColor : "transparent"}>
                    <Flex ml={6}>
                      <Nav label="Bridge" to="#" img={<Img src={BridgeIcon}/>}/>
                    </Flex>
                    {isOnBridge ? (<ChevronUpIcon mr={2}/>) : (<ChevronDownIcon mr={2}/>)}
                  </Flex>
                </NavLink>
                <Collapse animateOpacity in={isOnBridge}>
                  <Flex display={isOnBridge ? "" : "none"} flexDirection="column" ml={10} mb={3}>
                          <Flex ml={6} mb={3} onClick={onClose}>
