...
import { ModalOverlay, ModalContent, Modal, ModalCloseButton, ModalHeader, useColorModeValue, Box, Flex, Text, Button, useClipboard, Tooltip, IconButton, Link, Image, } from "@chakra-ui/react";
...
const WalletModal = ({ displayWallet, accounts, setDisplayWallet }) => {
    ...
    return (<>
      <Modal isCentered isOpen={displayWallet} onClose={() => setDisplayWallet(false)}>
        <ModalOverlay />
        <ModalContent width="95vw" borderRadius="6px" paddingBottom="20px" bgColor={bgColor} minHeight="40vh">
          <ModalHeader fontSize="24px" fontWeight="regular">Wallet</ModalHeader>
          <ModalCloseButton bg="none" size={'sm'} mt={3} mr={3} cursor="pointer" _focus={{ outline: 'none' }} onClick={() => setDisplayWallet(false)} p={'7px'} border={'1px solid'}/>
          <Box width="90%" margin="0 auto" fontSize="14px">
            <Flex border={`1px solid ${borderColor}`} borderRadius="6px" padding="4px 0" justifyContent="space-between">
              <Button variant={'ghost'} leftIcon={<StatusIcon connector={connector}/>}>
        ...
        : chainId === 56 || chainId === 97
            ...
            <Box>
              <Flex justifyContent="space-between" my="6">
                <Button background="transparent" border={`1px solid ${activeButtonColor}`} box-sizing="border-box" box-shadow="0px 1px 7px rgba(41, 45, 50, 0.08)" border-radius="6px" padding="23px 0" color={activeButtonColor} isFullWidth _hover={{ background: `${activeButtonColor}`, color: "#fff" }} onClick={() => {
        GSwitchWallet();
        setDisplayNetwork(state => !state);
                ...
                <Button border={`1px solid ${buttonColor}`} box-sizing="border-box" box-shadow="0px 1px 7px -2px rgba(24, 39, 75, 0.06), 0px 2px 2px rgba(24, 39, 75, 0.06)" border-radius="6px" padding="23px 0" color={buttonColor} _hover={{ background: `${buttonColor}`, color: "#fff" }} isFullWidth ml="4" background="transparent" onClick={disconnectWallet}>
                  Disconnect Wallet
                </Button>
              </Flex>
            </Box>
