...
const NetworkModal = ({ displayNetwork, setDisplayNetwork, }) => {
    ...
    const { login } = useAuth();
    const connectWallet = (connectorID, name) => {
    ...
    return (<>
      <Modal isOpen={displayNetwork} onClose={() => setDisplayNetwork(false)} isCentered>
        <ModalOverlay />
        <ModalContent width="90vw" borderRadius="6px" border={'1px solid'} borderColor={bgColor3} minHeight="40vh" boxShadow={shadow} bg={bg}>
          <ModalCloseButton bg="none" size={'sm'} mt={6} mr={3} cursor="pointer" _focus={{ outline: 'none' }} onClick={() => setDisplayNetwork(false)} p={'7px'} border={'1px solid'} borderColor={buttonBorder}/>
