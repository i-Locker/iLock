...
import { Menu, MenuButton, MenuItem, MenuList, Text, Link, Stack, useColorModeValue, Img, Box, Flex, IconButton, } from "@chakra-ui/react";
...
function DappsDropdown() {
    ...
    return (<>
      <Menu>
        <MenuButton as={IconButton} mr={1} transition="all 0.2s" borderRadius="md" borderWidth="1px" icon={<AiOutlineAppstore />} rightIcon={<ChevronDownIcon />} _hover={{ bg: "gray.100" }} _focus={{ boxShadow: "outline" }} fontSize="20px" className="HeaderDApps" p={2}/>
        <MenuList background={background} width="408px" boxShadow="0px 4px 20px -4px rgba(0, 0, 0, 0.1)" borderRadius="6px" boxSizing="border-box" padding="16px 16px 24px 16px">
          {swapItems.map((item, index) => {
        return (<MenuItem key={index} _hover={{ bg, color }} borderRadius={"6px"}>
