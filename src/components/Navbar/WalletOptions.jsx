...
const WalletItem = ({ name, image, connect }) => {
    const borderColor = useColorModeValue("lightBg.200", "darkBg.100");
    return (<Flex h="50px" cursor="pointer" _hover={{ border: '1px solid #4CAFFF' }} alignItems="center" p={9} my={4} border={'1px solid'} borderColor={borderColor} borderRadius={'6px'} onClick={() => connect()}>
...
const WalletOptions = ({ connect }) => {
    ...
    return (<>
            ...
            <ModalBody mt={4}>
                {walletItems.map((item, i) => (<WalletItem key={i} name={item.name} image={item.image} connect={() => item.connect(item.id, item.name)}/>))}
            </ModalBody>
        </>);
};
