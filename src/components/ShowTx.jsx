import { CHAIN_ID_ALGORAND, CHAIN_ID_AURORA, CHAIN_ID_AVAX, CHAIN_ID_BSC, CHAIN_ID_CELO, CHAIN_ID_ETH, CHAIN_ID_FANTOM, CHAIN_ID_KLAYTN, CHAIN_ID_KARURA, CHAIN_ID_NEAR, CHAIN_ID_MOONBEAM, CHAIN_ID_OASIS, CHAIN_ID_POLYGON, CHAIN_ID_SOLANA, CHAIN_ID_ACALA, isTerraChain, CHAIN_ID_TERRA2, CHAIN_ID_XPLA, CHAIN_ID_APTOS, CHAIN_ID_ARBITRUM, CHAIN_ID_INJECTIVE, CHAIN_ID_BASE, CHAIN_ID_OPTIMISM, CHAIN_ID_SEPOLIA, CHAIN_ID_SUI, } from "@certusone/wormhole-sdk";
...
export default function ShowTx({ chainId, tx, }) {
    const classes = useStyles();
    const showExplorerLink = CLUSTER === "testnet" ||
        (CLUSTER === "devnet" &&
            (chainId === CHAIN_ID_SOLANA || isTerraChain(chainId)));
    const explorerAddress = chainId === CHAIN_ID_ETH
        ? `https://${CLUSTER === "testnet" ? "goerli." : ""}etherscan.io/tx/${tx?.id}`
        : chainId === CHAIN_ID_SEPOLIA
            ? `https://${CLUSTER === "testnet" ? "sepolia." : ""}etherscan.io/tx/${tx?.id}`
                                ...
                                : chainId === CHAIN_ID_FANTOM
                                    ...
                                    : chainId === CHAIN_ID_KLAYTN
                                        ...
                                        : chainId === CHAIN_ID_CELO
                                            ? `https://${CLUSTER === "testnet" ? "alfajores.celoscan.io" : "explorer.celo.org"}/tx/${tx?.id}`
                                            : chainId === CHAIN_ID_KARURA
                                                ? `https://${CLUSTER === "testnet"
                                                    ? "blockscout.karura-dev.aca-dev.network"
                                                    : "blockscout.karura.network"}/tx/${tx?.id}`
                                                : chainId === CHAIN_ID_ACALA
                                                    ? `https://${CLUSTER === "testnet"
                                                        ? "blockscout.acala-dev.aca-dev.network"
                                                        : "blockscout.acala.network"}/tx/${tx?.id}`
                                                                ...
                                                                : chainId === CHAIN_ID_MOONBEAM
                                                                    ...
                                                                    : chainId === CHAIN_ID_BASE
                                                                        ...
                                                                        : chainId === CHAIN_ID_OPTIMISM
                                                                            ? `https://${CLUSTER === "testnet" ? "goerli-" : ""}optimism.etherscan.io/tx/${tx?.id}`
                                                                            : chainId === CHAIN_ID_XPLA
                                                                                ? `https://explorer.xpla.io/${CLUSTER === "testnet" ? "testnet/" : ""}tx/${tx?.id}`
                                                                                : chainId === CHAIN_ID_ARBITRUM
                                                                                    ...
                                                                                    : chainId === CHAIN_ID_APTOS
                                                                                        ...
                                                                                        : chainId === CHAIN_ID_SUI
                                                                                            ? `https://explorer.sui.io/txblock/${tx?.id}${CLUSTER === "testnet"
                                                                                                ? "?network=testnet"
                                                                                                : CLUSTER === "devnet"
                                                                                                    ? "?network=local"
                                                                                                    : ""}`
    ...
    return (<div className={classes.tx}>
      <Typography noWrap component="div" variant="body2">
        {tx.id}
      </Typography>
      {showExplorerLink && explorerAddress ? (<Button href={explorerAddress} target="_blank" rel="noopener noreferrer" size="small" variant="outlined" className={classes.viewButton}>
