import { Button, CircularProgress, createStyles, Dialog, DialogContent, DialogTitle, IconButton, List, ListItem, makeStyles, TextField, Tooltip, Typography, } from "@material-ui/core";
...
export const BasicAccountRender = (account, nft, displayBalance) => {
    ...
    const nftContent = (<div className={classes.tokenOverviewContainer}>
    ...
    const tokenContent = (<div className={classes.tokenOverviewContainer}>
      <div className={classes.tokenImageContainer}>
        {uri && <img alt="" className={classes.tokenImage} src={uri}/>}
      </div>
      <div>
...
export default function TokenPicker({ value, options, RenderOption, onChange, isValidAddress, getAddress, disabled, resetAccounts, nft, chainId, error, showLoader, useTokenId, }) {
    const classes = useStyles();
    ...
    const handleSelectOption = useCallback(async (option) => {
        ...
        try {
            ...
            if (e.message?.includes("v1")) {
                ...
                setSelectionError("Unable to retrieve required information about this token. Ensure your wallet is connected, then refresh the list.");
            }
        }
    ...
    const searchFilter = useCallback((option) => {
        if (!holderString) {
            return true;
        }
    ...
    const localFind = useCallback((address, tokenIdHolderString) => {
    ...
    useEffect(() => {
        ...
        if (isValidAddress(holderString, chainId)) {
            ...
            setLoadingError("");
            getAddress(holderString, useTokenId ? tokenIdHolderString : undefined).then((result) => {
                if (!cancelled) {
      ...
      <DialogContent className={classes.dialogContent}>
        ...
        {isLocalLoading || showLoader ? (localLoader) : loadingError || selectionError ? (displayLocalError) : (<List component="div" className={classes.tokenList}>
            {nonFeaturedOptions.map((option) => {
                return (<ListItem component="div" button onClick={() => handleSelectOption(option)} key={option.publicKey + option.mintKey + (option.tokenId || "")} disabled={getIsTokenTransferDisabled(chainId, targetChain, option.mintKey)}>
                  <RenderOption account={option}/>
                </ListItem>);
            })}
            {nonFeaturedOptions.length ? null : (<div className={classes.alignCenter}>
    ...
    const selectionChip = (<div className={classes.selectionButtonContainer}>
      <Button onClick={openDialog} disabled={disabled} variant="outlined" startIcon={<KeyboardArrowDownIcon />} className={classes.selectionButton}>
        {value ? (<RenderOption account={value}/>) : (<Typography color="textSecondary">Select a token</Typography>)}
      </Button>
    </div>);
    return (<>
      {dialog}
