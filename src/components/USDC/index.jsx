import { CHAIN_ID_AVAX, CHAIN_ID_ETH, getEmitterAddressEth, getSignedVAAWithRetry, isEVMChain, keccak256, parseSequenceFromLogEth, parseVaa, tryUint8ArrayToNative, uint8ArrayToHex, } from "@certusone/wormhole-sdk";
import { Container, FormControlLabel, FormGroup, makeStyles, Slider, Step, StepLabel, Stepper, Switch, Typography, } from "@material-ui/core";
...
import { selectAllowanceError, selectAmount, selectBalance, selectEstimatedSwapAmount, selectIsRedeemComplete, selectIsRedeeming, selectIsSending, selectMaxSwapAmount, selectRelayerFee, selectShouldApproveUnlimited, selectShouldRelay, selectSourceChain, selectSourceTxConfirmed, selectSourceTxHash, selectTargetChain, selectTargetTxHash, selectToNativeAmount, selectTransferInfo, } from "../../store/usdcSelectors";
import { setAllowanceError, setAmount, setBalance, setEstimatedSwapAmount, setIsRedeemComplete, setIsRedeeming, setIsSending, setMaxSwapAmount, setRelayerFee, setShouldRelay, setSourceChain, setSourceTxConfirmed, setSourceTxHash, setTargetChain, setTargetTxHash, setToNativeAmount, setTransferInfo, } from "../../store/usdcSlice";
...
function findCircleMessageInLogs(logs, circleEmitterAddress) {
    for (const log of logs) {
        if (log.address === circleEmitterAddress) {
            const messageSentIface = new ethers.utils.Interface([
                "event MessageSent(bytes message)",
            ]);
            return messageSentIface.parseLog(log).args.message;
        }
    }
    return null;
}
...
async function getCircleAttestation(messageHash, timeout = 2000) {
    while (true) {
        // get the post
...
async function handleCircleMessageInLogs(logs, circleEmitterAddress) {
...
const CIRCLE_DOMAIN_TO_WORMHOLE_CHAIN = {
    0: CHAIN_ID_ETH,
    1: CHAIN_ID_AVAX,
};
const USDC_RELAYER = {
...
const USDC_WH_EMITTER = {
    ...
    [CHAIN_ID_AVAX]: getEmitterAddressEth(USDC_WH_INTEGRATION[CHAIN_ID_AVAX] || ""),
};
function USDC() {
    ...
    const humanReadableTransferAmount = transferAmountParsed && formatUnits(transferAmountParsed, USDC_DECIMALS);
    ...
    const { isReady, statusMessage } = useIsWalletReady(transferInfo ? targetChain : sourceChain);
    const { signer, signerAddress } = useEthereumProvider();
    const shouldLockFields = isSending || isSendComplete || isRedeeming || isRedeemComplete;
    ...
    const handleSliderChange = useCallback((event, value) => {
        dispatch(setToNativeAmount(parseUnits(value.toString(), USDC_DECIMALS).toString()));
    }, [dispatch]);
    ...
    useEffect(() => {
        ...
        try {
            const parsedSourceChain = CHAINS_BY_ID[parseInt(pathSourceChain || "")]?.id;
            const parsedTargetChain = CHAINS_BY_ID[parseInt(pathTargetChain || "")]?.id;
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (cancelled)
                return;
            const parsedTokenAccount = await ethTokenToParsedTokenAccount(token, signerAddress);
            if (cancelled)
                return;
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        (async () => {
            const contract = new Contract(sourceRelayContract, [
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        (async () => {
            const contract = new Contract(targetRelayContract, [
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const estimatedSwap = await contract.calculateNativeSwapAmountOut(targetAsset, debouncedToNativeAmount);
            if (cancelled)
                return;
    ...
    useEffect(() => {
        ...
        if (!vaa)
            return;
        if (!targetCircleIntegrationContract)
            return;
        if (!isReady)
            return;
        if (!signer)
        ...
        (async () => {
            ...
            while (!wasRedeemed && !cancelled) {
                try {
                    const contract = new Contract(targetCircleIntegrationContract, [
                    ...
                    console.error("An error occurred while checking if the message was consumed", e);
                }
            }
            if (!cancelled) {
    ...
    // );
    const { sufficientAllowance, isAllowanceFetching, isApproveProcessing, approveAmount, } = useAllowance(sourceChain, sourceAsset, transferAmountParsed || undefined, false, shouldRelay ? sourceRelayContract : sourceContract);
    ...
    const isApproveDisabled = !isReady ||
        !amount ||
        !!amountError ||
        isAllowanceFetching ||
        isApproveProcessing;
        ...
        return () => {
            ...
            approveAmount(BigInt(transferAmountParsed)).then(() => {
                ...
                enqueueSnackbar(null, {
                    content: (<Alert severity="success">Approval transaction confirmed</Alert>),
                });
            }, (error) => dispatch(setAllowanceError("Failed to approve the token transfer.")));
        };
    ...
    const handleTransferClick = useCallback(() => {
        ...
        if (shouldRelay) {
          ...
          bytes32 targetRecipientWallet
            ...
            (async () => {
                try {
                    const tx = await contract.transferTokensWithRelay(sourceAsset, transferAmountParsed, toNativeAmount, targetChain, hexZeroPad(signerAddress, 32));
                    ...
                    enqueueSnackbar(null, {
                        content: (<Alert severity="success">Transfer transaction confirmed</Alert>),
                    });
                    // find circle message
                    ...
                    const seq = parseSequenceFromLogEth(receipt, getBridgeAddressForChain(sourceChain));
                    const { vaaBytes } = await getSignedVAAWithRetry(WORMHOLE_RPC_HOSTS, sourceChain, sourceRelayEmitter, seq);
                    // TODO: more discreet state for better loading messages
                    dispatch(setTransferInfo({
            ...
            const contract = new Contract(sourceContract, [
            ...
            (async () => {
                try {
                    const tx = await contract.depositForBurn(transferAmountParsed, targetDomain, hexZeroPad(signerAddress, 32), sourceAsset);
                    dispatch(setSourceTxHash(tx.hash));
                    const receipt = await tx.wait();
                    dispatch(setSourceTxConfirmed(true));
        ...
        if (shouldRelay) {
            ...
            (async () => {
                ...
                uint256 toNativeAmount
                    ...
                    const token = tryUint8ArrayToNative(payloadArray.subarray(1, 33), toChain);
                    const toNativeAmount = ethers.utils.hexlify(payloadArray.subarray(180, 212));
                    const nativeSwapQuote = await contract.calculateNativeSwapAmountOut(token, toNativeAmount);
                    ...
                    enqueueSnackbar(null, {
                        content: (<Alert severity="success">Redeem transaction confirmed</Alert>),
                    });
                }
                catch (e) {
                    console.error(e);
            ...
            (async () => {
                try {
                    ...
                    const tx = await contract.receiveMessage(transferInfo.circleBridgeMessage, transferInfo.circleAttestation);
                    dispatch(setTargetTxHash(tx.hash));
                    const receipt = await tx.wait();
                    if (!receipt) {
                        throw new Error("Invalid receipt");
      ...
      <Container maxWidth="xs">
        ...
        <div className={classes.chainSelectWrapper}>
          <div className={classes.chainSelectContainer}>
            <img src={CHAINS_BY_ID[sourceChain].logo} alt={CHAINS_BY_ID[sourceChain].name} className={classes.chainLogo}/>
            <Typography>Source</Typography>
          </div>
          <div className={classes.chainSelectArrow}>
            <ChainSelectArrow onClick={handleSwitch} disabled={shouldLockFields}/>
          </div>
          <div className={classes.chainSelectContainer}>
            <img src={CHAINS_BY_ID[targetChain].logo} alt={CHAINS_BY_ID[targetChain].name} className={classes.chainLogo}/>
            <Typography>Target</Typography>
          </div>
        </div>
        <NumberTextField variant="outlined" label="Amount (USDC)" fullWidth className={classes.transferField} value={amount} onChange={handleAmountChange} disabled={shouldLockFields} onMaxClick={balance && balance.uiAmountString ? handleMaxClick : undefined}/>
        ...
        <FormGroup className={classes.toggle}>
          <FormControlLabel disabled={shouldLockFields} control={<Switch value={shouldRelay} onChange={handleToggleRelay} color="primary"/>} label="Use relayer"/>
        </FormGroup>
        <div className={classes.infoContainer}>
          <Typography variant="body2" style={{ flexGrow: 1 }} color={shouldRelay ? "textPrimary" : "textSecondary"}>
            Relayer Fee
          </Typography>
          <Typography variant="body2" color={shouldRelay ? "textPrimary" : "textSecondary"}>
        ...
        <div className={classes.sliderContainer}>
          <Slider disabled={!shouldRelay || shouldLockFields} onChange={handleSliderChange} value={Number(formatUnits(toNativeAmount, USDC_DECIMALS))} step={0.001} min={0} max={Number(formatUnits(maxSwapAmount || 0, USDC_DECIMALS))} valueLabelDisplay="off"/>
        </div>
        <div className={classes.infoContainer}>
          <Typography variant="body2" style={{ flexGrow: 1 }} color={shouldRelay ? "textPrimary" : "textSecondary"}>
        ...
        <div className={classes.infoContainer}>
          ...
          <Typography variant="body2" color={shouldRelay ? "textPrimary" : "textSecondary"}>
            {(estimatedSwapAmount &&
            `${formatUnits(estimatedSwapAmount, METAMASK_CHAIN_PARAMETERS[getEvmChainId(targetChain) || 1]
                ?.nativeCurrency.decimals || 18)} ${METAMASK_CHAIN_PARAMETERS[getEvmChainId(targetChain) || 1]
                ?.nativeCurrency.symbol || "ETH"}`) ||
            null}
          </Typography>
        </div>
        {transferInfo ? (<ButtonWithLoader disabled={!isReady || isRedeeming || isRedeemComplete} onClick={handleRedeemClick} showLoader={isRedeeming} error={statusMessage}>
            Redeem
          </ButtonWithLoader>) : approveButtonNeeded ? (<>
            {/* <FormControlLabel
            control={
              <Checkbox
          ...
          /> */}
            <ButtonWithLoader disabled={isApproveDisabled} onClick={shouldApproveUnlimited ? approveUnlimited : approveExactAmount} showLoader={isAllowanceFetching || isApproveProcessing} error={errorMessage || amountError}>
              {"Approve " +
                (shouldApproveUnlimited
                    ? "Unlimited"
          ...
          </>) : (<ButtonWithLoader disabled={!isReady || isSending} onClick={handleTransferClick} showLoader={isSending} error={statusMessage || amountError}>
            Transfer
          </ButtonWithLoader>)}
        {!statusMessage && !amountError ? (<Typography variant="body2" className={classes.message}>
            {isApproveProcessing ? ("Waiting for wallet approval and confirmation...") : isSending ? (!sourceTxHash ? ("Waiting for wallet approval...") : !sourceTxConfirmed ? ("Waiting for tx confirmation...") : ("Waiting for Circle attestation...")) : isRedeeming ? (!targetTxHash ? ("Waiting for wallet approval...") : ("Waiting for tx confirmation...")) : (<>&nbsp;</>)}
          </Typography>) : null}
        <div className={classes.stepperContainer}>
          <Stepper activeStep={isRedeemComplete
            ? 3
            : transferInfo
                ? 2
                : approveButtonNeeded
