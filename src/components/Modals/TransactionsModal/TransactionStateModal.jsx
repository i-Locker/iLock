...
import { Text, Spinner, ModalCloseButton, ModalContent, ModalOverlay, Modal, ModalBody, useColorModeValue, Circle, Button, Link } from "@chakra-ui/react";
...
const TransactionStateModal = () => {
    ...
    const modalDetails = useSelector((state) => state.application.modal);
    ...
    return (<>
            <Modal isOpen={setOpen} onClose={handleCloseModal} isCentered>
                <ModalOverlay />
                <ModalContent bg={bgColour} color="#fff" borderRadius="6px" paddingBottom="15px" width="95vw">
                    <ModalCloseButton bg="none" color={closeBtnColour} cursor="pointer" _focus={{ outline: 'none' }} onClick={handleCloseModal} border={'1px solid'} size={'sm'} mt={3} mr={3} p={'7px'}/>
                    <ModalBody align="center" my={2}>

                        {modalDetails?.trxState === TrxState.WaitingForConfirmation ?
        <Spinner thickness="4px" speed="0.53s" emptyColor="transparent" color="#319EF6" size="xl" width="100px" height="100px" my={10}/> : modalDetails?.trxState === TrxState.TransactionSuccessful ?
        ...
        modalDetails?.trxState === TrxState.TransactionFailed ?
            <>
                                    <Text py={3} fontSize="14px" fontWeight="normal" color="#008DFF">
                                        <a href="#" target="_blank">
                                            Retry
                                        </a>

                                    </Text>
                                    <Text py={3} fontSize="14px" fontWeight="normal" color={errorBgColour}>
                                        {modalDetails?.message}

                                    </Text>
                                </>
            : modalDetails?.trxState === TrxState.TransactionSuccessful ? <Text py={3} fontSize="14px" fontWeight="normal" color="#008DFF">
        ...
        <Button variant="brand" padding="24px 0" width="100%" isFullWidth boxShadow="none" border="0" mt={3} background={closeButtonBgColour} color="#FFFFFF" cursor="pointer" onClick={handleCloseModal}>
                                Close
                            </Button>}
