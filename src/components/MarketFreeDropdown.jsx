...
const MarketFreeDropDown = ({ marketType, setMarketType, chainID, switchMarket, type }) => {
    ...
    useEffect(() => {
        ...
        if (chainID === 97)
            setMarketArray(binanceTestFreeMarketArray);
        else if (chainID === 137)
            setMarketArray(polygonFreeMarketArray);
        else if (chainID === 43113 || chainID === 43114)
            setMarketArray(AvalancheFreeMarketArray);
        else if (chainID === 80001)
            setMarketArray(polygonTestFreeMarketArray);
    }, [chainID]);
    const borderColor = useColorModeValue('#DEE6ED', '#324D68');
    return (<Menu>
        <MenuButton variant="ghost" as={Button} transition="all 0.2s" rightIcon={<ChevronDownIcon />} fontWeight={200} _focus={{ color: "#319EF6" }} fontSize="13px" textTransform={'capitalize'} border={`2px solid ${borderColor}`}>
          <Flex>
        ...
        <MenuList>
          {marketArray.map((item, index) => (<MenuItem key={index} _focus={{ color: "#319EF6" }} onClick={() => {
        console.log({ item, marketArray });
        // setMarketType(item.name)
        // switchMarket(item.name.toLowerCase())
    }} fontSize="13px">
           <Img src={`./images/${item.image}`} width="30px" height="30px" mr={2}/>
           {item.name}
          </MenuItem>))}
