...
import { ModalOverlay, ModalContent, Modal, ModalCloseButton, ModalBody, ModalHeader, useDisclosure, useColorModeValue, Flex, Text, Button, Image } from "@chakra-ui/react";
...
const NewToken = ({ tokens, open, setDisplayImportedToken, handleCurrencySelect }) => {
    ...
    return (<>
        <Modal isOpen={open} onClose={onClose} isCentered>
            ...
            <ModalContent width="95vw" borderRadius="6px" bgColor={bgColor}>
                 <ModalCloseButton bg="none" size={'sm'} mt={3} mr={3} cursor="pointer" _focus={{ outline: 'none' }} onClick={() => setDisplayImportedToken(false)} p={'7px'} border='1px solid'/>
          <ModalHeader color={heavyTextColor} fontWeight="500" boxShadow={`0px 1px 0px ${boxShadow}`}>
            <ArrowBackIcon w={10} h={6} cursor="pointer" marginLeft="-12px" onClick={() => setDisplayImportedToken(false)}/>
              ...
              <Text marginLeft="0px" marginTop="22px" fontSize="16px">
                ...
                <ModalBody pb="30px">
                  ...
                  <Text textAlign="center" color={heavyTextColor}>
                    ...
                    {tokens?.map((token, id) => {
        return (<Flex justifyContent="center" alignItems="center" flexDirection="column" bgColor={boxColor} borderRadius="6px" my="3" py="25px" key={id}>
                ...
                <Button color="white" bgColor={textColor} isFullWidth mt="3" height="48px" onClick={() => {
        tokens?.map(token => {
            addToken(token);
