...
import { useColorModeValue, Box, Flex, Text, Switch, Button, Link } from "@chakra-ui/react";
import "./tooltip.css";
...
const ManageList = ({ setListURL, setTokenList, setOpenImportToken, }) => {
    ...
    const ListRow = memo(function ListRow({ listUrl }) {
        ...
        if (!list)
            return null;
        return (<Flex justifyContent="space-between" p="2" py="4" cursor="pointer" bgColor={isActive ? selectedList : boxColor} border={`1px solid ${isActive ? borderColor : borderColor2}`} borderRadius="6px" my="4">
        <Box mt="2" mr="3">
          {list.logoURI ? (<ListLogo size="40px" squared logoURI={list.logoURI} alt={`${list.name} list logo`}/>) : (<div style={{ width: '24px', height: '24px', marginRight: '1rem' }}/>)}
        </Box>

        <Flex width="80%">
          <Box mt="3">
            <Text color={heavyTextColor} fontWeight="700" fontSize="15px">{list.name}
            </Text>
            <Flex>
              <Text color={lightTextColor} fontSize="12px">{list.tokens.length} Tokens  </Text>
              <Box className="tooltip">
                ...
                <Box fontSize="16px" className="tooltiptext" background={boxColor2}>
                  ...
                  <Box>
                    <Button mt="3" p="2" onClick={handleRemoveList} backgroundColor={backgroundColor} color={color}>
                      Delete
                    </Button>
                    {pending && (<Button variant="text" onClick={handleAcceptListUpdate} style={{ fontSize: '12px' }}>
                        Update list
                      </Button>)}
                  </Box>
                </Box>
        ...
        <Box mt="2">
          <Switch size="lg" py="1" defaultChecked={isActive} colorScheme={switchColor} onChange={() => {
            if (isActive) {
                handleDisableList();
            }
            else {
    ...
    return (<>
      <Box pt="2">
        <ModalInput placeholder="https:// or ipfs:// or ENS name" searchQuery={tokenInput} changeInput={handleInput}/>
      </Box>
      {addError ? (<Text color="red.400" my="2" style={{ textOverflow: 'ellipsis', overflow: 'hidden' }}>
          {addError}
        </Text>) : null}

      <Box margin="0px auto" mt="4">
        {tempList && (<Flex justifyContent="space-between">
            <Flex>
            ...
            <Box>
              {isImported ? (<Box>
                ...
                </Box>) : (<Button width="fit-content" onClick={handleImport}>
                  Import
                </Button>)}
            </Box>
          </Flex>)}
