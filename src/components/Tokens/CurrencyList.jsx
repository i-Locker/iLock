...
const CurrencyList = ({ currency, onCurrencySelect, selectedCurrency, otherSelectedCurrency, }) => {
    ...
    return (<Flex justifyContent='space-between' py='2' fontSize='16px' cursor={isSelected ? "" : "pointer"} onClick={() => (isSelected ? null : handleSelect())} opacity={isSelected || otherSelected ? "0.6" : "1"} _hover={{ background: `${isSelected ? "" : hover}` }} px='4' borderRadius='10px'>
      <Flex>
        <Box mr={3} mt={3}>
        ...
        <Box>
          ...
          {!currency.isNative && !isOnSelectedList && customAdded ? (<Text>{currency.name} • Added by user</Text>) : (<Text color={lightTextColor}>{currency.name} </Text>)}
