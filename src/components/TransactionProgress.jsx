import { CHAIN_ID_ACALA, CHAIN_ID_ARBITRUM, CHAIN_ID_AURORA, CHAIN_ID_BASE, CHAIN_ID_CELO, CHAIN_ID_FANTOM, CHAIN_ID_KARURA, CHAIN_ID_KLAYTN, CHAIN_ID_MOONBEAM, CHAIN_ID_OASIS, CHAIN_ID_OPTIMISM, CHAIN_ID_POLYGON, CHAIN_ID_SOLANA, isEVMChain, } from "@certusone/wormhole-sdk";
...
export default function TransactionProgress({ chainId, tx, isSendComplete, }) {
    const classes = useStyles();
    const { provider } = useEthereumProvider();
    const [currentBlock, setCurrentBlock] = useState(0);
    useEffect(() => {
        if (isSendComplete || !tx)
            return;
    ...
    const blockDiff = tx && tx.block && currentBlock ? currentBlock - tx.block : undefined;
    ...
    if (!isSendComplete &&
        ...
        <LinearProgress value={blockDiff < expectedBlocks ? (blockDiff / expectedBlocks) * 75 : 75} variant="determinate"/>
