...
import { selectNFTIsSourceComplete, selectNFTShouldLockFields, selectNFTSourceBalanceString, selectNFTSourceChain, selectNFTSourceError, } from "../../store/selectors";
    ...
    return (<>
      <StepDescription>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <div>
            <Button component={Link} to="/nft-origin-verifier" size="small" variant="outlined" startIcon={<VerifiedUser />}>
              NFT Origin Verifier
            </Button>
          </div>
        </div>
      </StepDescription>
      <ChainSelect variant="outlined" select fullWidth value={sourceChain} onChange={handleSourceChange} disabled={shouldLockFields} chains={CHAINS_WITH_NFT_SUPPORT}/>
      {isEVMChain(sourceChain) ? (<Alert severity="info" variant="outlined">
          Only NFTs which implement ERC-721 are supported.
        </Alert>) : null}
      {sourceChain === CHAIN_ID_SOLANA ? (<Alert severity="info" variant="outlined">
          Only NFTs with a supply of 1 are supported.
        </Alert>) : null}
      <KeyAndBalance chainId={sourceChain}/>
      {isReady || uiAmountString ? (<div className={classes.transferField}>
          <TokenSelector disabled={shouldLockFields} nft={true}/>
        </div>) : null}
      <LowBalanceWarning chainId={sourceChain}/>
      <ChainWarningMessage chainId={sourceChain}/>
      <ButtonWithLoader disabled={!isSourceComplete || isTransferDisabled} onClick={handleNextClick} showLoader={false} error={statusMessage || error}>
        Next
      </ButtonWithLoader>
    </>);
}
