import { CHAIN_ID_AVAX, CHAIN_ID_BSC, CHAIN_ID_ETH, CHAIN_ID_FANTOM, CHAIN_ID_KLAYTN, CHAIN_ID_NEON, CHAIN_ID_OASIS, CHAIN_ID_POLYGON, ethers_contracts, } from "@certusone/wormhole-sdk";
...
import { errorDataWrapper, fetchDataWrapper, getEmptyDataWrapper, receiveDataWrapper, } from "../store/helpers";
import { WAVAX_ADDRESS, WAVAX_DECIMALS, WBNB_ADDRESS, WBNB_DECIMALS, WETH_ADDRESS, WETH_DECIMALS, WFTM_ADDRESS, WFTM_DECIMALS, WKLAY_ADDRESS, WKLAY_DECIMALS, WMATIC_ADDRESS, WMATIC_DECIMALS, WNEON_ADDRESS, WNEON_DECIMALS, WROSE_ADDRESS, WROSE_DECIMALS, } from "../utils/consts";
...
function UnwrapNative() {
    ...
    const [selectedChainId, setSelectedChainId] = useState(CHAIN_ID_ETH);
    const [balances, setBalances] = useState(getEmptyDataWrapper());
    const [unwrapRequest, setUnwrapRequest] = useState(getEmptyDataWrapper());
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (cancelled)
                    return;
                const wrappedToken = await ethers_contracts.MockWETH9__factory.connect(supportedTokens[selectedChainId].address, signer);
                if (cancelled)
                    return;
                ...
                if (cancelled)
                    return;
                setBalances(errorDataWrapper("An error occurred while fetching balances"));
            }
        })();
        return () => {
            cancelled = true;
    ...
    return (<Container maxWidth="md">
      ...
      <Paper className={classes.mainPaper}>
        ...
        <TextField select value={selectedChainId} onChange={handleSelect} className={classes.select} disabled={unwrapRequest.isFetching}>
          {Object.entries(supportedTokens).map(([key, item]) => (<MenuItem key={key} value={key}>
              <ListItemIcon className={classes.listItemIcon}>
                <img src={item.icon} alt={item.symbol} className={classes.icon}/>
              </ListItemIcon>
              {item.symbol}
            </MenuItem>))}
        </TextField>
        <Typography variant="h5" gutterBottom>
          {formatUnits(balances.data?.wrapped || 0, supportedTokens[selectedChainId].decimals)}
        </Typography>
        <Typography variant="subtitle1" gutterBottom>
          {supportedTokens[selectedChainId].symbol.substring(1)}
        </Typography>
        <Typography variant="h5" gutterBottom>
          {formatUnits(balances.data?.native || 0, supportedTokens[selectedChainId].decimals)}
        </Typography>
        <Typography variant="h5" gutterBottom></Typography>
        <ButtonWithLoader disabled={!isReady ||
            balances.isFetching ||
            !balances.data ||
            balances.data.wrapped.eq(0) ||
            unwrapRequest.isFetching} onClick={handleClick} showLoader={balances.isFetching || unwrapRequest.isFetching} error={error}>
          Unwrap All
        </ButtonWithLoader>
      </Paper>
    </Container>);
