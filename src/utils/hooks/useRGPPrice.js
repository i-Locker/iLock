...
export const useRGPPrice = () => {
    ...
    const [RGPPrice, setRGPPrice] = useState(0);
    useEffect(() => {
        const getRGPprice = async () => {
            try {
                const RGPBUSDToken = chainId === 43114 || chainId === 43113 ? null : await smartSwapLPTokenPoolOne(SMARTSWAPLP_TOKEN1ADDRESSES[chainId], library);
