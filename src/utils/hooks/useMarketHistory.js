...
const useMarketHistory = (socket) => {
    ...
    const getTransactionFromDatabase = async () => {
        ...
        const transactions = transaction.reduce((array, item) => {
            const successfullyTransaction = item.transaction.filter((x) => x.transactionHash !== "" && x.status === 1);
            ...
            try {
                ...
                if (location.includes("/swap")) {
                    ...
                    const filteredData = jsonData.result
                        .filter((items) => decodeInput(items.input, SmartSwapRouter02) !== undefined && items.isError !== "1")
                        .map((items) => ({
                    ...
                    const marketData = filteredData.map((data) => ({
                    ...
                    if (transaction.length > 0) {
                        ...
                        if (locationData === "auto") {
                            let data = transaction.filter((data) => data.typeOfTransaction === "Auto Time")
                                .sort((a, b) => b.time - a.time);
                            result = data.filter((item) => item.status === 1 && parseInt(item.chainID) === chainId);
                        }
                        else if (locationData === "price") {
                            let data = transaction.filter((data) => data.typeOfTransaction === "Set Price")
                                .sort((a, b) => b.time - a.time);
                            result = data.filter((item) => item.status === 1 && parseInt(item.chainID) === chainId);
                        }
                        dataToUse = await Promise.all(result.map(async (data) => {
                            // try{
                            let dataBase = {
                ...
                const marketHistory = marketSwap.map((data) => ({
                    ...
                    amountIn: data.tokenIn.name === SupportedChainName[data.chainID] ? parseInt(formatAmount(data.amountIn, data.tokenIn.decimals)) / data.totalTransaction : formatAmount(data.amountIn, data.tokenIn.decimals),
