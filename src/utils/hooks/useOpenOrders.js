...
const useOpenOrders = (socket) => {
    ...
    const getTransactionFromDatabase = async (address) => {
        ...
        const transactions = transaction[0].transaction.filter((item) => item.status === 2 || item.status === 3);
        return transactions;
    };
            ...
            try {
                ...
                if (transaction.length > 0) {
                    ...
                    if (locationData === "auto") {
                        data = transaction.filter((data) => data.typeOfTransaction === "Auto Time");
                    }
                    else if (locationData === "price") {
                        data = transaction.filter((data) => data.typeOfTransaction === "Set Price");
                    }
                    const result = data.filter((item) => item.errorArray.length === 0 && item.transactionHash === "" && parseInt(item.chainID) === chainId);
                ...
                const marketHistory = marketSwap.map((data) => ({
                    ...
                    token2Icon: data.tokenOut && getTokenSymbol(data.tokenOut.symbol),
                    token1: data.tokenIn,
                    token2: data.tokenOut,
                    amountIn: data.tokenIn && data.tokenIn?.name === SupportedChainName[data.chainID] ? parseFloat(formatAmount(data.amountIn, data.tokenIn.decimals)) / data.totalTransaction : data.tokenIn && formatAmount(data.amountIn, data.tokenIn.decimals),
