...
import { BUSD, MASTERCHEFNEWLPADDRESSES, RGPADDRESSES, SMARTSWAPFACTORYADDRESSES, SMARTSWAPLP_TOKEN3ADDRESSES, SYMBOLS, USDC, USDT, } from "../addresses";
import { getERC20Token, useProvider } from "../utilsFunctions";
import { farmSection } from "../../pages/FarmingV2";
export const useGetNewFarms = (id, reload, setReload) => {
    ...
    const getLpfarmAddresses = async (farmLength, contract) => {
    ...
    const getBNBPrice = async () => {
        try {
            let BNBPrice;
            const pair = await smartSwapLPTokenPoolThree(SMARTSWAPLP_TOKEN3ADDRESSES[ChainId], lib);
    ...
    const calculateRigelPrice = async () => {
        try {
            let rgpPrice;
            const pair = await smartFactory(SMARTSWAPFACTORYADDRESSES[ChainId], lib);
    ...
    const getLpTokenReserve = async (address) => {
        ...
        try {
            ...
            const [totalReserves, token0, token1, LPLockedPair, LPTotalSupply] = await Promise.all([
                ...
                LpTokenContract.balanceOf(MASTERCHEFNEWLPADDRESSES[ChainId][id]),
    ...
    const formatSymbol = async (symbol) => {
        let formatted;
        if (symbol === "WMATIC") {
            formatted = "MATIC";
        }
    ...
    const calculateLiquidityandApy = async (address, rgpPrice, BNBPrice, pid, reward, Lp) => {
        try {
            const masterchef = await MasterChefV2Contract(MASTERCHEFNEWLPADDRESSES[ChainId][id], lib);
            ...
            const LiquidityLocked = parseFloat(ethers.utils.formatEther(reserves.LPSupply)) /
            ...
            return {
                id: pid,
                img: "rgp.svg",
                deposit: `${await formatSymbol(symbol0)}-${await formatSymbol(symbol1)}`,
    ...
    const loopFarms = async (LpAddress, rgpPrice, BNBPrice) => {
        const data = [];
        for (let i = 0; i < LpAddress.length; i++) {
            const farm = await calculateLiquidityandApy(LpAddress[i], rgpPrice, BNBPrice, i, ChainId === 137 || ChainId === 80001
                ? 2014.83125
