...
import { smartFactory, LiquidityPairInstance, SmartSwapRouter, } from "../Contracts";
    ...
    useEffect(() => {
        const loadUserPairs = async () => {
            if (account) {
                setLoading(true);
                try {
                    const SmartFactory = await smartFactory(SMARTSWAPFACTORYADDRESSES[chainId], library);
                    ...
                    const pairsData = await Promise.all(allExchange.map((address) => getPoolData(address, account, chainId, library)));
                    const userPairs = pairsData.filter((pair) => parseFloat(pair.poolToken) !== 0);
...
const getPoolData = async (address, account, chainId, library) => {
    const liquidity = await LiquidityPairInstance(address, library);
    const [balance, totalSupply, reserves, token0, token1, allowance] = await Promise.all([
...
const getPooledToken = (params) => {
    //construct decimal
    const Decimal = 10 ** params.decimals;
    //init fraction class
    const fraction = new Fraction(params.balance.toString(), params.totalSupply.toString());
...
export const useGetLiquidityById = async (address1, address2, hasBeenApproved, loadData) => {
    const { account, chainId, library } = useActiveWeb3React();
    const [loading, setLoading] = useState(true);
    const [LiquidityPairData, setLiquidityPairData] = useState();
        ...
        const loadPair = async () => {
            if (account) {
                ...
                try {
                    ...
                    const PairData = await getPoolData(Pair, account, chainId, library);
                    setLoading(false);
...
export const getAddress = (currency) => {
    return currency?.isNative
        ? WNATIVEADDRESSES[currency?.chainId]
        : currency?.wrapped.address;
};
export const filterPools = ({ TokenA, TokenB, liquidities, }) => {
    let tokenA = getAddress(TokenA);
    let tokenB = getAddress(TokenB);
    GFindLiquidity(tokenA, tokenB);
    return liquidities?.filter((liquidity) => (liquidity.path[0].fromPath === tokenA &&
        liquidity.path[1].toPath === tokenB) ||
        (liquidity.path[0].fromPath === tokenB &&
            liquidity.path[1].toPath === tokenA));
};
export const useTokenValueToBeRemoved = ({ pool, inputValue, }) => {
    return useMemo(() => {
        // const percent = new Percent
        if (pool && inputValue) {
            const poolToken0Fraction = (pool.pooledToken0 / 100) * parseInt(inputValue);
            ...
            const poolToken1Fraction = (pool.pooledToken1 / 100) * parseInt(inputValue);
            ...
            const fixTokenFraction = parseFloat(inputValue) === 100
...
export const useIsPoolsAvailable = (CurrencyA, CurrencyB) => {
    const { chainId, account, library } = useActiveWeb3React();
    const [pairAvailable, setPairAvailable] = useState(false);
    useMemo(async () => {
        if (CurrencyA && CurrencyB && account) {
            const factory = await smartFactory(SMARTSWAPFACTORYADDRESSES[chainId], library);
...
export const usePoolShare = (CurrencyA, CurrencyB) => {
    const { chainId, account, library } = useActiveWeb3React();
...
export const usePricePerToken = (CurrencyA, CurrencyB) => {
    const { chainId, account, library } = useActiveWeb3React();
    const [priceAToB, setPriceAToB] = useState(undefined);
    const [priceBToA, setPriceBToA] = useState(undefined);
    useMemo(async () => {
        if (account && CurrencyA && CurrencyB) {
            const router = await SmartSwapRouter(SMARTSWAPROUTER[chainId], library);
            const factory = await smartFactory(SMARTSWAPFACTORYADDRESSES[chainId], library);
            const currencyAAddress = getAddress(CurrencyA);
            const currencyBAddress = getAddress(CurrencyB);
            const pair = await factory.getPair(currencyAAddress, currencyBAddress);
            if (pair !== "0x0000000000000000000000000000000000000000") {
                try {
                    ...
                    const [PriceAToB, PriceBToA] = await Promise.all([
                        router.quote(ethers.utils.parseUnits("1", CurrencyA.decimals), token0 === currencyAAddress ? reserves[0] : reserves[1], token0 === currencyAAddress ? reserves[1] : reserves[0]),
                        router.quote(ethers.utils.parseUnits("1", CurrencyB.decimals), token0 === currencyAAddress ? reserves[1] : reserves[0], token0 === currencyAAddress ? reserves[0] : reserves[1]),
                    ]);
                    setPriceAToB(ethers.utils.formatUnits(PriceAToB.toString(), CurrencyB.decimals));
                    setPriceBToA(ethers.utils.formatUnits(PriceBToA.toString(), CurrencyA.decimals));
                }
                catch (err) {
...
export const useAllowance = (CurrencyA, CurrencyB, checkTokenApproval, inputA, inputB) => {
    const { account, chainId, library } = useActiveWeb3React();
    ...
    useMemo(async () => {
        if (CurrencyA && CurrencyB && account) {
            ...
            const [tokenA, tokenB] = await Promise.all([
                getERC20Token(currencyAAddress, library),
                getERC20Token(currencyBAddress, library),
            ]);
            const [allowanceA, allowanceB, decimalsA, decimalsB] = await Promise.all([
                tokenA.allowance(account, SMARTSWAPROUTER[chainId]),
                tokenB.allowance(account, SMARTSWAPROUTER[chainId]),
                tokenA.decimals(),
                tokenB.decimals()
            ]);
...
export const usePriceForNewPool = (inputA, inputB, pairExist) => {
...
const calculateTokenToBeMinted = (reservesa, reservesb, totalsupply, amounta, amountb) => {
    return Math.min((parseFloat(amounta) * parseFloat(totalsupply)) / parseFloat(reservesa), (parseFloat(amountb) * parseFloat(totalsupply)) / parseFloat(reservesb));
};
const calculatePoolShare = (minted, totalSupply) => {
    const supply = ethers.utils.formatEther(totalSupply);
    const value = parseFloat(supply) + minted;
    return (minted / value) * 100;
};
export const useMintedLiquidity = (CurrencyA, CurrencyB, AmountA, AmountB) => {
    const { chainId, account, library } = useActiveWeb3React();
    ...
    useMemo(async () => {
        if (account && CurrencyA && CurrencyB && AmountA && AmountB) {
            try {
                ...
                const minted = calculateTokenToBeMinted(reserves[0].toString(), reserves[1].toString(), totalSupply, AmountA, AmountB);
