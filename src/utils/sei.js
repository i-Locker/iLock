import { CHAIN_ID_SEI, buildTokenId, coalesceChainId, hexToUint8Array, isNativeCosmWasmDenom, } from "@certusone/wormhole-sdk";
import { getCosmWasmClient, getQueryClient } from "@sei-js/core";
import { fromUint8Array } from "js-base64";
import { SEI_CHAIN_CONFIGURATION } from "./consts";
export const getSeiWasmClient = () => getCosmWasmClient(SEI_CHAIN_CONFIGURATION.rpcUrl);
export const getSeiQueryClient = () => getQueryClient(SEI_CHAIN_CONFIGURATION.restUrl);
/**
 ...
 */
export async function getForeignAssetSei(tokenBridgeAddress, cosmwasmClient, originChain, originAsset) {
    try {
        const queryResult = await cosmwasmClient.queryContractSmart(tokenBridgeAddress, {
            wrapped_registry: {
 ...
 */
export async function getIsTransferCompletedSei(tokenBridgeAddress, signedVAA, client) {
 ...
 * @returns Information about the asset
 */
export async function getOriginalAssetSei(wrappedAddress, client) {
...
export const queryExternalIdSei = async (client, tokenBridgeAddress, externalTokenId) => {
    try {
        ...
        const denomOrAddress = response.token_id.Bank?.denom ||
 ...
 */
export async function submitVAAOnSei(signedVAA) {
    return {
