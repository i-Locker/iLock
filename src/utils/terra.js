import { cosmos, CHAIN_ID_TERRA2, terra, } from "@certusone/wormhole-sdk";
...
export const getNativeTerraIcon = (symbol) => symbol === LUNA_SYMBOL
    ? `https://assets.terra.money/icon/svg/LUNA.png`
    : symbol === LUNA_CLASSIC_SYMBOL
        ? `https://assets.terra.money/icon/svg/LUNC.svg`
        : `https://assets.terra.money/icon/60/${symbol.slice(0, symbol.length - 1)}.png`;
export const formatNativeDenom = (denom, chainId) => {
...
export const formatTerraNativeBalance = (balance = "") => formatUnits(balance, 6);
export async function waitForTerraExecution(transaction, chainId) {
...
export const isValidTerraAddress = (address, chainId) => {
    ...
    try {
        ...
        const isLengthOk = isParseable.length === (chainId === CHAIN_ID_TERRA2 ? 32 : 20);
...
export async function postWithFees(wallet, msgs, memo, feeDenoms, chainId) {
