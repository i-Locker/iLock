...
export default function hub({ chainState, tabValue, setTabValue }) {
    ...
    useEffect(() => {
        async function __STATE(chainState) {
    ...
    useEffect(() => {
        async function __LOAD(token1, token2, inputAmount) {
            if (token2 === "select") {
                setSwapBtnState(0);
                return;
            }
        ...
        __LOAD(token1, token2, inputAmount);
    }, [token1, token2, inputAmount]);

    useEffect(() => {
        async function ___TOKENIZE(token1, account) {
            let token1Inst = new web3.eth.Contract(TokenABI, token1.address);
