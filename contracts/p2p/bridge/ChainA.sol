...
contract iVault_ChainA is iAuth, IRECEIVE_KEK {
    ...
    function withdraw() external virtual override {
        ...
        Vault storage VR_d = vaultRecords[address(_development)];
        uint256 ETH_liquidity = uint256(address(this).balance);
        uint256 sTb = ETH_liquidity;
        require(synced(sTb, address(this), false) == true);
        (uint256 sumOfLiquidityWithdrawn, uint256 cliq, uint256 dliq) = split(
            ETH_liquidity
        );
        VR_c.community.coinAmountDrawn += uint256(cliq);
        VR_d.development.coinAmountDrawn += uint256(dliq);
        VR_c.community.coinAmountOwed = uint256(0);
        VR_d.development.coinAmountOwed = uint256(0);
        ...
        returns (bool)
    {
        ...
        uint256 sTb = IERC20(KEK).balanceOf(address(this));
    ...
    function transfer(
        ...
        assert(address(receiver) != address(0));
        uint256 sTb = address(this).balance;
        require(synced(sTb, address(this), false) == true);
        ...
        (, uint256 cliq, uint256 dliq) = split(uint256(amount));
        uint256 cTok = cliq;
        uint256 dTok = dliq;
        VR_c.community.coinAmountOwed -= uint256(cliq);
        VR_d.development.coinAmountOwed -= uint256(dliq);
        VR_c.community.coinAmountDrawn += uint256(cTok);
        VR_d.development.coinAmountDrawn += uint256(dTok);
