...
contract KEKLocker is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    string private tokenName = "KekChain LP Locker";
    /// @notice Changeable symbol for the share token.
    string private tokenSymbol = "KEK_LP_LOCKER";
    ...
    function createLock(
        ...
        address payable holdingContract = payable(
            address(new HoldingContract())
        );
        ...
        // approve token (msg.sender => this) on UI
        token.safeTransferFrom(msg.sender, holdingContract, amount);
        // approve token (holdingContract => this)
        set_locker_allowance(lockId, amount);
    ...
    function withdraw(uint256 lockId) external nonReentrant {
        ...
        address payable holdingContract = lock.holdingContract;
        require(
        ...
        HoldingContract(holdingContract).transferTo(token, msg.sender, amount);

        emit Withdraw(lockId, address(lock.token), msg.sender, amount);
    }

    function _set_locker_allowance(uint256 lockId, uint256 amount)
        public
        onlyOperator
        returns (bool success)
    {
        return set_locker_allowance(lockId, amount);
    }

    function set_locker_allowance(uint256 lockId, uint256 amount)
        internal
        nonReentrant
        returns (bool success)
    {
        ...
        IERC20 token = lock.token;
        address payable holdingContract = lock.holdingContract;
        (success) = HoldingContract(holdingContract).setAllowance(
            token,
            payable(this),
            amount
        );
        require(success, "Failed to set initial allowance");
        return success;
    }
    ...
    function transferOperator(address payable newOperator)
        external
        onlyOwner
        nonReentrant
    {
        require(newOperator != operator, "already set");
        address payable oldOperator = operator;
...
/**
 * @title KEKChain's HoldingContract to manage individually locked positions.
 * @notice The HoldingContract stores an individually locked position, it can only be unlocked by the main locker address, which should be a KEKChain locker.
 * @author Muse
 */
contract HoldingContract {
    ...
    // optional, add stacked token, or create ERC20
    // string private tokenName = "KekChain LP Holder";
    // string private tokenSymbol = "KEK_LP_Holder";
